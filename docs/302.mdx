...
| 设备是否列表         | 设备是否列表上传，``false`` 时每个设备实体都会单独发布，注意性能需求，默认为 ``true``             |  |
| 变量是否列表         | 变量是否列表上传，``false`` 时每个变量实体都会单独发布，注意性能需求，默认为 ``true``             |  |
| 报警是否列表         | 报警是否列表上传，``false`` 时每个报警实体都会单独发布，注意性能需求，默认为 ``true``             |  |
